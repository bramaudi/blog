...
class Category_Model extends Database {
	...
	{
		$public = $public ? 'WHERE status = 1': '';
		return $this->num_rows("SELECT COUNT(id) FROM tb_categories ".$public);
	}
	...
	public function list($public = 0)
	{
		$public = $public ? 'WHERE status = 1 ': '';
		$this->query("SELECT * FROM tb_categories ".$public." ORDER BY name");
		$this->execute();
